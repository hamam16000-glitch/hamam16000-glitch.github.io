...
function calc(){var bal=n('current'),r=n('rate')/100,years=Math.max(0,Math.floor(n('years'))),annual=n('annual');for(var i=0;i<years;i++){bal=bal*(1+r)+annual}var contrib=n('current')+annual*years;set('mainResult',money(bal));set('contrib',money(contrib));set('growth',money(bal-contrib));set('withdrawal',money(bal*0.04));}
